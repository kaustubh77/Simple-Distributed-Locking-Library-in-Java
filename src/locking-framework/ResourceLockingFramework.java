...
public final class ResourceLockingFramework {
    ...
    public boolean lockResource(String resourceLockId, String ownerId, String operation) {

        return lockResource(
                resourceLockId,
                ownerId,
                operation,
                resourceLocksConfig.getRecoverySystemResourceLockTimeoutLimit(),
                resourceLocksConfig.getRecoverySystemLockRetryTime());
    }

    public boolean lockRecoveryAppliance(String raConnectString, String ownerId) {

        return lockResource(
                raConnectString,
                ownerId,
                "RA_CONNECT",
                resourceLocksConfig.getRecoveryApplianceResourceLockTimeoutLimit(),
                resourceLocksConfig.getRecoveryApplianceLockRetryTime());
    }

    public boolean lockDataPath(
            String vcnId, String recoveryApplianceId, String ownerId, String operation) {

        Preconditions.checkArgument(!Strings.isNullOrEmpty(vcnId), "VCN ID is required");

        Preconditions.checkArgument(
                !Strings.isNullOrEmpty(recoveryApplianceId), "Recovery Appliance ID is required");
        ...
        long timeoutTime =
                System.currentTimeMillis()
                        + resourceLocksConfig.getDataPathResourceLockTimeoutLimit();

        long retryTime = resourceLocksConfig.getDataPathLockRetryTime();

        do {
            if (getLockOnResource(
                    ResourceLockEntity.getShortenedResourceLockId(vcnId + recoveryApplianceId),
                    ownerId,
                    operation)) {
            ...
            // Early return if data path is already created by some other process
            if (operation.equals(ResourceLockOperationTypes.CREATE_DATA_PATH)
                    && dataPathsDao.getDataPathId(vcnId, recoveryApplianceId) != null) {
    ...
    public int unlockRecoveryAppliance(String raConnectString, String ownerId) {
        return unlockResource(raConnectString, ownerId);
    }
